...
function converter(options) {
  ...
  */
  ...
  if (options._.length > 0) {
    ...
    console.log("No input file path provided");
    // console.log(process.stdin);
    //Read the input from stdin and convert it
    const stdinInput = process.stdin;
    const inputChunks = [];
    stdinInput.resume();
    stdinInput.setEncoding('utf8');
    stdinInput.on('data', function (chunk) {
      inputChunks.push(chunk);
    });

    stdinInput.on('end', function () {
      input_sarif_file = inputChunks.join()
    });

    // process.stdin.pipe(parser(options));
